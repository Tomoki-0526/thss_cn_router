...
ArpCache::periodicCheckArpRequestsAndCacheEntries()
{
  ...
  // FILL THIS IN
  /* 记录发送次数为5的request */
  std::vector<std::list<std::shared_ptr<ArpRequest>>::iterator> invalid_requests;
  for (auto iter = m_arpRequests.begin(); iter != m_arpRequests.end();++iter) {
    if ((*iter)->nTimesSent == 5) {
      invalid_requests.push_back(iter);
    }
  }
  ...
  for (auto req : m_arpRequests) {
    ...
    if (req->nTimesSent == 5) {
      ...
      req->timeSent = current_time;
      req->nTimesSent++;
    }
  }

  /* 删除发送次数超过5次的request */
  for (auto iter : invalid_requests) {
    m_arpRequests.remove(*iter);
  }

  /* 记录invalid的cache entry */
  std::vector<std::list<std::shared_ptr<ArpEntry>>::iterator> invalid_entries;
  ...
  /* 删除invalid的cache entry */
  for (auto iter : invalid_entries) {
